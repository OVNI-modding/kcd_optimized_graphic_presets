...
for pak in Path('mod/Data').glob('**/*.pak'):
	if pak.is_dir():
		...
		pakage.close()
		zout.writestr( str(Path('Data', packagePrefix+pak.name)), gamePackage.getvalue() )
...
# IMPORTANT
#The new mods format doesn't allow to create cvargroup, and thus is totally uncompatible with this mod
exit()
